...
# Dipolo
q_dipolo = 6e-4
new_Ex3, new_Ey3 = getField(X, Y, -q_dipolo, [0, -0.1], k_e)
new_Ex4, new_Ey4 = getField(X, Y, q_dipolo, [0, 0.1], k_e)
Ex += new_Ex3 + new_Ex4
Ey += new_Ey3 + new_Ey4

ndip_locx = find_nearest_indx(x, 0)
ndip_locy = find_nearest_indx(y, -0.1)

pdip_locx = find_nearest_indx(x, 0)
pdip_locy = find_nearest_indx(y, 0.1)

nFx = -q_dipolo*Ex[ndip_locx][ndip_locy]
nFy = -q_dipolo*Ey[ndip_locx][ndip_locy]

pFx = q_dipolo*Ex[pdip_locx][pdip_locy]
pFy = q_dipolo*Ey[pdip_locx][pdip_locy]
...
ax.quiver(X, Y, ex, ey)
ax.quiver(0, 0, nFx, nFy, color='blue', scale=580000)
ax.quiver(0, 0, pFx, pFy, color='red', scale=580000)
for qloc_x in qloc_x_range:
    ax.scatter(qloc_x, qloc_y, c='red', s=100)
    ax.scatter(0, (qloc_x/5)+6, c='blue', s=100)

ax.scatter(0, -0.1, c='blue', s=33)
ax.scatter(0, 0.1, c='red', s=33)
